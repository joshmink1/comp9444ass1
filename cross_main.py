...
import argparse
from cross import Full3Net, Full4Net, DenseNet
...
parser.add_argument('--net',type=str,default='full3',help='full3, full4 or dense')
...
if list(net.parameters()):
    ...
    plt.savefig('./plot/cross.png',format='png')
            ...
            if layer < 3 or args.net == 'full4':
                for node in range(args.hid):
                    ...
                    plt.savefig('./plot/hid_%s_%d_%d_%d.png' \
                         % (args.net, args.hid, layer, node),format='png')

    # graph output unit
    graph_output(net)
    plt.scatter(full_input[:,0],full_input[:,1],
            c=1-full_target[:,0],cmap='RdYlBu')
    plt.savefig('./plot/out_%s_%d.png' %(args.net, args.hid),format='png')
